...
def test_nothing_is_sent_into_the_night_anywhere_in_a_day(db):
    """Единственная жёсткая гарантия продукта, проверенная по всем суткам.

    Не «в норме», а во всех тридцати часах подряд, в четырёх поясах, включая
    два по разные стороны линии дат. Настройка часа намеренно выкручена в край:
    `delivery_hour` зажимает 23 до 21, окно ловит человека до полуночи по его
    часам — и всё, что после 22:00, обязано быть отказано.
    ...
    awake = [moment for moment in composed if rules.is_quiet(moment)]
    assert awake == [], f"уведомление посреди ночи: {awake}"
