...
#: Сколько дорогих расчётов один аккаунт может запросить в минуту.
#:
#: **Дырой был `/v1/systems/*`: у него нет никакого потолка.** Всё, что стоит
#: денег, отмеряется на `user.id`, но расчёт денег не стоит — он бесплатен по
#: закону продукта, — и потому не считался вовсе. А процессорное время он стоит:
#: годовой скан транзитов — 1.26 с в конечном пуле `app.thread_pool_size()`,
#: синастрия дороже (два натала плюс сравнение). Кэш не спасает: достаточно
#: менять широту на сотую градуса, и каждый запрос — промах и новый скан
#: (`calc.contract.cache_key` берёт координаты как есть). Один токен, десяток
#: параллельных сканов — и пул занят целиком, а с ним встают и главы, и чат, и
#: проверка живости у всех. 20 августа 2026 замерено: 20 параллельных годовых
#: сканов с одного гостя — 54–61 с каждый против 2.67 с одиночного.
#:
#: Считаем по `user.id`, а не по источнику: строку `user` уже стережёт
#: `guest_mint` (30 в час на источник), так что ротация токенов упирается в
#: него, а этот потолок закрывает вторую половину — один токен не должен
#: выносить пул. Число щедрое к живому человеку: экран «Сегодня» открывает
#: пару сканов, оглавление главы — ещё несколько; двадцать в минуту переживёт
#: любой обычный сценарий и упрётся только под заливом.
SYSTEMS_CALCS_PER_MINUTE = 20
SYSTEMS_WINDOW_SECONDS = 60.0


async def throttle_expensive_calc(
    request: Request, user: CurrentUser, session: SessionDep
) -> None:
    """Потолок на дорогой расчёт, по аккаунту. Зависимость дорогих `/systems/*`.

    Стоит зависимостью, а не строкой в теле маршрута, чтобы отказ случился
    **до** `resolve_birth` и `_run`: смысл в том, чтобы дорогая работа не
    началась. `user` здесь — тот же объект, что получит маршрут (FastAPI
    кэширует `current_user` в пределах запроса), так что гостя это не заводит
    второй раз и потолок не касается самого входа.

    Отказ — `HTTPException`, `session_scope` откатит увеличение вместе с
    транзакцией; в базе останется ровно столько, сколько сканов было пропущено
    (тот же довод, что у `SharedWindow.hit`).
    """
    ok = await window(
        request,
        "systems_calc",
        limit=SYSTEMS_CALCS_PER_MINUTE,
        seconds=SYSTEMS_WINDOW_SECONDS,
    ).hit(session, f"user:{user.id}")
    if not ok:
        log.warning(
            "expensive calc refused: account asked for more than %d scans a minute",
            SYSTEMS_CALCS_PER_MINUTE,
        )
        raise HTTPException(
            status.HTTP_429_TOO_MANY_REQUESTS,
            detail={
                "error": "calc_rate_limit",
                "message": (
                    "too many calculations at once — give it a moment and try again"
                ),
            },
            headers={"Retry-After": str(int(SYSTEMS_WINDOW_SECONDS))},
        )
...
async def partner_profile_id(
    ...
    **Строгий разбор количества остаётся мягким, но названный id проверяется.**
    Развилка «партнёров ноль» / «партнёров несколько, назови» по-прежнему живёт
    в `readings._partner` и обязана срабатывать *после* проверки прав — иначе
    неоплативший человек с двумя партнёрами получит 422 вместо пейволла и не
    узнает, что глава вообще продаётся. Но **существование и владение** названным
    id — другое: без этой проверки подписчик, приславший `partner_profile_id`
    несуществующего или чужого профиля, тратил включённый месячный кредит и писал
    бесполезный грант `pair:{id}` в первой транзакции `readings.read`, а 404
    прилетал из второй (`_partner`) — уже после коммита списания. Найдено
    20.08.2026. Проверка та же, что в `_partner` и `load_profile`: «не найдено ==
    не твоё» → 404, чтобы id нельзя было перебирать.
    ...
    """
    if stated:
        profile = await session.get(Profile, stated)
        if profile is None or profile.user_id != user.id:
            raise HTTPException(status.HTTP_404_NOT_FOUND, detail="no such profile")
        return stated
