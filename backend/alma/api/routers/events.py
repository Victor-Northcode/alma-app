...
#: остаётся: он про другое — про размер таблицы под одним идентификатором. Два
#: потолка, две разные вещи под защитой; снят один — второй ещё стоит.
#:
#: **Оба теперь переживают перезапуск и общие для всех воркеров.** Прежняя
#: оговорка на этом месте — «чего счётчик в памяти не умеет» — перестала быть
#: верной: счёт по источнику лежит в `rate_window` (см. `deps.SharedWindow`), и
#: восемь процессов больше не дают восьмикратной нормы.
...
async def record_stage(
    ...
    **A caller that changes its anonymous id between beacons breaks the funnel
    ...
    # это одна строка `rate_window` вместо `count(*)` по таблице событий, и, в
    # отличие от счёта по `anon_id`, он не снимается новым значением заголовка.
    # Тот, кто норму уже выбрал, после этой строки не заставляет нас ещё и
    # считать строки таблицы — а начиная с 19.08.2026 ещё и не ходит в базу
    # вовсе: `SharedWindow` помнит выбранный потолок до конца окна.
    if user is None and not await window(
        request,
        "anon_events",
