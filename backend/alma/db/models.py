...
class ChatMessage(Base):
    ...
    #: Язык, на котором эта реплика написана. Заведён 28.08.2026 вместе с
    #: переводом бесед: чтобы перевести тред на язык приложения, надо знать,
    #: какие реплики уже на нём.
    #:
    #: **Ставится только у ответов Alma.** Её реплику язык запроса описывает
    #: честно — `conversation._wrong_script` не даст ей ответить на другом.
    #: А человек пишет на чём хочет: русский вопрос при английском интерфейсе
    #: — обычное дело, и штамп языка приложения на нём вечно закрывал бы ему
    #: перевод (кеш пропускает реплики, чей язык совпал с запрошенным).
    #: Null — «язык неизвестен», честное значение и для всякой строки до
    #: колонки, и для всякого вопроса человека; такие реплики переводятся
    #: наравне с чужой локалью — модель, которой велено вернуть уже-целевой
    #: текст без изменений, делает ровно это, и ответ ложится в кеш.
...
class FriendInvite(Base):
    """Приглашение «проверь нас»: ссылка, которой один человек зовёт другого.

    Единица роста продукта (владелец, 31.08.2026: «делай друзей» — цикл
    Co-Star, перенесённый на восемь систем): пригласивший делится ссылкой,
    приглашённый вводит на веб-странице свою дату, и совместимость появляется
    **у обоих** — у каждого в аккаунте рождается профиль второго. Ссылка
    одноразовая, как magic link: приглашение — жест к одному человеку, а
    многоразовая ссылка в публичном чате рожала бы у пригласившего толпу
    незнакомых «друзей».

    Токен ходит в URL открытым текстом и потому не хэшируется, в отличие от
    magic link: ссылка входит НЕ даёт — приём создаёт связь, но не пускает в
    чужой аккаунт, и утёкшая ссылка стоит максимум одного нежеланного друга,
    которого можно удалить.

    `claimed_*` заполняются приёмом и делают повторный приём той же ссылки
    тем же человеком идемпотентным: второй POST отвечает тем же результатом,
    а не ошибкой — двойное нажатие «Готово» не должно быть видно.
    """

    __tablename__ = "friend_invite"

    id: Mapped[str] = mapped_column(String(32), primary_key=True, default=new_id)
    #: То, что уходит в ссылку. Свой столбец, а не `id`: id светится в логах
    #: и админке, а токен — это капабилити, и жить им должно раздельно.
    token: Mapped[str] = mapped_column(String(43), unique=True, index=True)
    inviter_user_id: Mapped[str] = mapped_column(
        ForeignKey("user.id", ondelete="CASCADE"), index=True
    )
    #: Имя, которым страница приглашения подписана. Снимок на момент выдачи:
    #: страница живёт без токена аккаунта и не имеет права ходить в профиль.
    inviter_name: Mapped[str | None] = mapped_column(String(120))

    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)
    claimed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    #: Кто принял. НЕ внешний ключ: аккаунт принявшего может удалиться, а у
    #: пригласившего остаётся обычный профиль-друг, и history приёма не
    #: обязана умирать вместе с чужим аккаунтом.
    claimed_by_user_id: Mapped[str | None] = mapped_column(String(32), index=True)
    #: Профили, рождённые приёмом: друг в аккаунте пригласившего и
    #: пригласивший в аккаунте друга. Обычные строки, не ключи — профиль
    #: можно удалить (дружба расторгается удалением профиля), а строка
    #: приглашения остаётся историей.
    inviter_profile_id: Mapped[str | None] = mapped_column(String(32))
    claimer_profile_id: Mapped[str | None] = mapped_column(String(32))
