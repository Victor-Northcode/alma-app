...
REGIONAL_CENTS: dict[str, dict[str, int]] = {
    ...
    # Рубль — полоса страницы «Как оплатить» (Т-Банк, ТЗ владельца от
    # 29.08.2026: «$9.99 в месяц или эквивалент в рублях»). Точки выведены по
    # тому же правилу x99 из курса ~92 ₽/$ на 29.08.2026: 999 ₽ подписка,
    # 499 ₽ дверь и пара, 1 999 ₽ бандл. **Выведенное — не утверждённое**
    # (правило шапки этого блока): владелец обязан подтвердить четыре числа
    # до открытия продаж — здесь они и меняются, одним коммитом.
