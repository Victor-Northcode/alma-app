...
PRODUCTS: dict[str, Product] = {
    ...
    # ── Пачки вопросов (01.09.2026, разбор витрины Co-Star: «дешёвый первый
    # платёж — половина их магазина»). Расходуемые: покупаются сколько угодно
    # раз, кучки складываются. `slug` кладёт размер прямо в систему гранта —
    # "questions:5" — потому что у Entitlement нет колонки количества, а
    # размер обязан пережить любую смену каталога: сколько купили, столько и
    # останется. Тратятся ПОСЛЕ включённых порций (месячной подписки,
    # приветственных) и на сильной модели — купленное не сгорает, пока
    # действует включённое, и отвечает тем же голосом, что платные главы.
    # Цены $2.99/$5.99/$7.99 — точки Co-Star, ВЫВЕДЕННЫЕ, владелец не
    # подтверждал (правило шапки REGIONAL_CENTS).
    "questions.5": Product(
        "questions:5", "5 questions to Alma", "consumable", 299,
        band="q5", scope="questions",
    ),
    "questions.10": Product(
        "questions:10", "10 questions to Alma", "consumable", 599,
        band="q10", scope="questions",
    ),
    "questions.25": Product(
        "questions:25", "25 questions to Alma", "consumable", 799,
    ...
    # есть год. Цена $12.99 — точка Co-Star, ВЫВЕДЕННАЯ, не подтверждена.
    "report.year": Product(
...
#: Полосы q5/q10/q25 (пачки вопросов) и year («Год вперёд») добавлены
#: 01.09.2026 и целиком ВЫВЕДЕНЫ из точек Co-Star через пропорции своих
#: рынков, с окончаниями, принятыми в каждой строке. Владелец не
#: подтверждал ни одного из этих чисел: подтвердить — до сохранения в
#: консолях магазинов, тем же правилом, что и полоса bundle выше.
