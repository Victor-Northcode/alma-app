...
class Product:
    ...
    #: Сколько дней живёт выданный этим товаром доступ. Ноль — бессрочно.
    #: Заведено под «Год вперёд» (01.09.2026): разовый отчёт по соляру
    #: актуален до следующего дня рождения, и грант обязан истечь сам —
    #: иначе одна покупка открывала бы соляры всех будущих лет. Расходуемый
    #: вид (`consumable`) — вторая половина того же решения: магазин обязан
    #: позволить купить следующий год.
    lifetime_days: int = 0
...
PRODUCTS: dict[str, Product] = {
    ...
    # ── Пачки вопросов (01.09.2026, разбор витрины Co-Star). Расходуемые:
    # покупаются сколько угодно раз, кучки складываются. `slug` кладёт размер
    # прямо в систему гранта — "questions:5" — потому что у Entitlement нет
    # колонки количества, а размер обязан пережить любую смену каталога.
    # Тратятся ПОСЛЕ включённых порций и на сильной модели.
    #
    # **Экономика лестницы — по поручению владельца 02.09.2026 («цены
    # продумай все… чтоб заманивала и нам выгодная»), четыре решения:**
    #
    # 1. Вход $1.99 — цена кофе: работа пачки — сделать человека ПЛАТЯЩИМ,
    #    и первый доллар важнее третьего. Себестоимость пяти ходов сильной
    #    моделью ≈ $0.39, после комиссии магазина остаётся ≥ $1.00.
    # 2. Штука дешевеет с размером: $0.40 → $0.35 → $0.28 за вопрос —
    #    иначе большая пачка наказывает за доверие.
    # 3. q25 за $6.99 стоит РЯДОМ с подпиской $9.99 нарочно: «25 вопросов —
    #    или вся Alma на месяц за три доллара сверху» — компромисс-декой,
    #    продающий подписку тем, кто пришёл за вопросами.
    # 4. Ядро полки (двери 4.99, пара 4.99, бандл 19.99, месяц 9.99) НЕ
    #    тронуто: это решённые точки владельца от 17.08.2026, и вся новая
    #    лестница построена вокруг них, а не поверх. Все точки US есть в
    #    сетке Apple. Менять — словом владельца, здесь и в GOLDEN разом.
    "questions.5": Product(
        "questions:5", "5 questions to Alma", "consumable", 199,
        band="q5", scope="questions",
    ),
    "questions.10": Product(
        "questions:10", "10 questions to Alma", "consumable", 349,
        band="q10", scope="questions",
    ),
    "questions.25": Product(
        "questions:25", "25 questions to Alma", "consumable", 699,
        band="q25", scope="questions",
    ),
    # «Год вперёд» — полный соляр разовой покупкой (01.09.2026): система
    # solar-return открывается на 366 дней — до следующего дня рождения,
    # к которому соляр и пересчитается. Расходуемый: следующий год покупается
    # снова. НЕ противоречит правилу «соляр продаётся только подпиской»
    # (NOT_A_DOOR): двери нет по-прежнему — бессрочного доступа не существует,
    # есть год. Цена $12.99 = 1.3 подписки: разовый год одной системы обязан
    # стоить дороже месяца всего — иначе он ест подписку; и меньше двух
    # месяцев — иначе его не за чем покупать. Подарочная форма продукта.
    "report.year": Product(
        "solar-return", "Your year ahead", "consumable", 1299,
        band="year", scope="system", lifetime_days=366,
    ),
}
...
#: Полосы q5/q10/q25/year выбраны 02.09.2026 по поручению владельца
#: («цены продумай все») — довод лестницы у самих товаров ниже. Локальные
#: точки держат пропорции своего рынка и его окончания, и в каждой валюте
#: сохраняют порядок лестницы: q5 < q10 < дверь < q25 < месяц < год <
#: бандл (стережёт test_the_ordering_of_prices_survives_the_currency).
REGIONAL_CENTS: dict[str, dict[str, int]] = {
    ...
    "EUR": {"door": 549, "pair": 549, "bundle": 2099, "monthly": 1049, "q5": 229, "q10": 399, "q25": 799, "year": 1399},
    # UK consumers pay roughly 8% above the US for digital subscriptions and
    # the market is used to it.
    "GBP": {"door": 499, "pair": 499, "bundle": 1999, "monthly": 999, "q5": 199, "q10": 349, "q25": 699, "year": 1299},
    ...
    "CHF": {"door": 590, "pair": 590, "bundle": 2390, "monthly": 1190, "q5": 290, "q10": 490, "q25": 890, "year": 1590},
    "AUD": {"door": 799, "pair": 799, "bundle": 3199, "monthly": 1599, "q5": 329, "q10": 549, "q25": 1099, "year": 1999},
    "CAD": {"door": 749, "pair": 749, "bundle": 2799, "monthly": 1399, "q5": 299, "q10": 499, "q25": 999, "year": 1899},
    "NOK": {"door": 5900, "pair": 5900, "bundle": 21900, "monthly": 10900, "q5": 2500, "q10": 4500, "q25": 8900, "year": 14900},
    "DKK": {"door": 3900, "pair": 3900, "bundle": 15900, "monthly": 7900, "q5": 1500, "q10": 2900, "q25": 5900, "year": 9900},
    ...
    "BRL": {"door": 1290, "pair": 1290, "bundle": 5190, "monthly": 2590, "q5": 590, "q10": 990, "q25": 1890, "year": 3290},
    "MXN": {"door": 5900, "pair": 5900, "bundle": 21900, "monthly": 10900, "q5": 2500, "q10": 4500, "q25": 8500, "year": 13900},
    "PLN": {"door": 1099, "pair": 1099, "bundle": 4399, "monthly": 2199, "q5": 499, "q10": 799, "q25": 1599, "year": 2699},
    "TRY": {"door": 6900, "pair": 6900, "bundle": 25900, "monthly": 12900, "q5": 2900, "q10": 4900, "q25": 9900, "year": 16900},
    "INR": {"door": 10900, "pair": 10900, "bundle": 43900, "monthly": 21900, "q5": 4500, "q10": 7900, "q25": 15900, "year": 26900},
    # Рубль — полоса страницы «Как оплатить» (Т-Банк). Четыре числа НАЗВАНЫ
    # ВЛАДЕЛЬЦЕМ 31.08.2026: «цены делай 449 449 1799 и 849» — дверь, пара,
    # бандл, месяц. Это уже не выведенное, а утверждённое; менять — только
    # новым словом владельца, здесь и в GOLDEN теста одним коммитом.
    "RUB": {"door": 44900, "pair": 44900, "bundle": 179900, "monthly": 84900, "q5": 16900, "q10": 29900, "q25": 59900, "year": 109900},
}
...
CURRENCY_FORMAT: dict[str, Notation] = {
    ...
    "TRY": Notation("₺", ".", ","),
    "INR": Notation("₹", ",", "."),
    # Знак после суммы, тонкая тысячная — «1 999 ₽», как пишет рубль.
    "RUB": Notation(" ₽", " ", ",", suffix=True),
}
...
COUNTRY_CURRENCY: dict[str, str] = {
    ...
    # Страны вне еврозоны, с которых сторы всё равно берут евро — снято с
    # живых консолей 24.08.2026 (список EUR-витрин у Apple и Play шире
    # еврозоны). Здесь они относились к умолчанию USD, и витрина с бэкендом
    # называли бы разные суммы: сербу Apple печатает 5,49 €, а каталог отвечал
    # $4.99. Одна валюта — одна цена, откуда бы человек ни платил.
    #
    # Оговорка про Косово (XK): у Apple его витрина существует только в
    # консоли — в API у неё НЕТ ни одной точки цены, ручную цену туда не
    # положить, и Apple держит свою автоконвертацию (9,99 € за подписку против
    # наших 10,49 €). Приложение это не рвёт: экран всегда печатает цену,
    # которую назвал сам магазин. Каталог обслуживает карточный фолбэк, и там
    # правило «одна валюта — одна цена» держится.
    #
    # Габон и Гвинея-Бисау — EUR-витрины Play (у Apple их сторфронты платят в
    # других валютах, но см. ту же оговорку: в приложении цена всегда цена
    # магазина).
    "BG": "EUR", "BA": "EUR", "XK": "EUR", "RS": "EUR", "ME": "EUR",
    "GA": "EUR", "GW": "EUR",
    "GB": "GBP",
    "CH": "CHF",
    # Россия платит в рублях через Т-Банк — карточный фолбэк страницы «Как
    # оплатить»; в приложении цена по-прежнему всегда цена магазина.
    "RU": "RUB",
    "AU": "AUD",
    "CA": "CAD",
    "NO": "NOK",
