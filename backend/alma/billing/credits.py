...
# ── бесплатный тизер «Притяжение», и его кап ───────────────────────────────
#
# Тизер — первая глава совместимости, бесплатная для всех (`free_chapters`).
# Он и есть главный крючок флоу P4: два абзаца про то, что тянет этих двоих
# друг к другу, написанные средней моделью примерно за 3¢, после которых
# предлагается полный разбор.
#
# Три цента — мало ровно до тех пор, пока их считают на одного человека.
# Свободный аккаунт, добавляющий партнёра за партнёром, — это генерация за
# генерацией из `free_month_budget`, и ни одна из них не приближает покупку:
# тот, кто проверяет десятого человека бесплатно, уже показал, что платить не
# собирается. Поэтому кап.
#
# **Кап — на человека в месяц, а не на запрос.** Повторный вход в уже
# проверенного партнёра не тратит ничего: тизер про него уже написан и лежит
# в `Reading`, второй раз он не генерируется вовсе. Считаются именно новые
# люди — то есть ровно то, что стоит денег.

#: Ключ в `Setting`, которым кап меняется **без релиза**.
#:
#: Это не украшение и не «на будущее»: правильное число здесь неизвестно до
#: первых живых данных, а деплой ради цифры — деплой в худший возможный момент,
#: когда цифра уже жжёт деньги. Значение — объект, а не число, чтобы завтра
#: рядом встал кап на другой тир и это не стало вторым ключом.
TEASER_SETTING = "pair.teaser_cap"

#: Сколько **разных людей** свободный аккаунт может проверить бесплатно за
#: календарный месяц, если в `Setting` ничего не сказано.
#:
#: Один. Решение владельца от 17.08.2026, и оно расходится с числом на холсте
#: («Three free previews this month») — холст устарел, прав владелец. Число
#: живёт здесь одной константой именно поэтому: два места, где написано «3» и
#: «1», — это экран, обещающий втрое больше, чем даёт сервер.
TEASER_CAP_DEFAULT = 1

#: Метрика в `UsageCounter`. Одна строка на (аккаунт, месяц, партнёр) —
#: считаются люди, а не запросы.
TEASER_METRIC = "pair_teaser"


def _teaser_row_id(user_id: str, profile_id: str) -> str:
    """Ключ строки счётчика — по (аккаунт, партнёр), **без месяца**.

    Месяц живёт в колонке `day`, а не в ключе, и это не мелочь размещения.
    С месяцем в ключе строка «этого человека уже показывали» существовала бы
    только внутри своего месяца — то есть тизер, написанный в январе, в
    феврале выглядел бы как новый, упирался бы в кап и **закрывался**. Отобрать
    уже написанный бесплатный текст нельзя: он у человека на экране, за него
    ничего не просили, и его исчезновение читается как поломка.

    Так что строка одна на человека навсегда: повтор бесплатен всегда, а к
    капу месяца относятся только те, кого раньше не было.

    Хэш, потому что `UsageCounter.id` — 64 символа, а «аккаунт + профиль» в
    открытом виде на длинных id не помещается; на Postgres это отказ вставки,
    на SQLite — молча обрезанный ключ, то есть два разных партнёра,
    схлопнутые в одну строку. Ключи этой таблицы нигде не разбираются
    обратно, так что читаемым ему быть незачем — только уникальным.
    """
    digest = hashlib.sha256(f"{user_id}|{profile_id}".encode()).hexdigest()
    return f"{TEASER_METRIC}:{digest[:40]}"


async def teaser_cap(session: AsyncSession) -> int:
    """Сколько бесплатных тизеров в месяц — из `Setting`, иначе константа выше.

    Кривое значение (не число, отрицательное, строка) игнорируется в пользу
    дефолта, и не молча: опечатка в оперативной настройке не должна ни открыть
    бесплатную генерацию всем, ни закрыть её всем, — а должна вернуть то
    поведение, которое было до правки, и оставить строку в логе.
    """
    row = await session.get(Setting, TEASER_SETTING)
    raw = (row.value or {}).get("per_month") if row is not None else None
    if isinstance(raw, bool) or not isinstance(raw, int) or raw < 0:
        if raw is not None:
            log.error(
                "%s holds %r, which is not a number of people per month — "
                "falling back to %s", TEASER_SETTING, raw, TEASER_CAP_DEFAULT,
            )
        return TEASER_CAP_DEFAULT
    return raw


async def teaser_allowed(
    session: AsyncSession, user: User, profile_id: str, *, at: datetime | None = None
) -> bool:
    """Можно ли написать бесплатный тизер про этого человека — и отметить, что написали.

    Возвращает `True` и **записывает** факт: разделять «спросить» и «отметить»
    значило бы иметь путь, на котором тизер сгенерирован, а счётчик нет, и
    таким путём становится любое исключение между двумя вызовами.

    Повтор про того же человека — всегда `True` и без роста счётчика, в любом
    месяце: строка уже есть, генерации не будет (глава лежит в `Reading`), и
    отказать здесь значило бы закрыть уже написанный бесплатный текст.
    """
    moment = at or utcnow()
    month = moment.date().replace(day=1)
    row_id = _teaser_row_id(user.id, profile_id)

    seen = await session.get(UsageCounter, row_id)
    if seen is not None:
        return True

    cap = await teaser_cap(session)
    used = await session.scalar(
        select(func.count())
        .select_from(UsageCounter)
        .where(
            UsageCounter.user_id == user.id,
            UsageCounter.metric == TEASER_METRIC,
            UsageCounter.day == month,
        )
    )
    if (used or 0) >= cap:
        log.info(
            "account %s has had %s free pair teaser(s) this month against a cap "
            "of %s — %s gets the paywall instead",
            user.id, used, cap, profile_id,
        )
        return False

    session.add(
        UsageCounter(
            id=row_id, user_id=user.id, day=month,
            metric=TEASER_METRIC, count=1, amount=0.0,
        )
    )
    await session.flush()
    return True
