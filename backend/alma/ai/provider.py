...
#:
#: **Живёт здесь, потому что дважды написанная — дважды сломанная.** Ступени
#: были продублированы в `ai/writer.py` и в `ai/conversation.py` одним и тем же
#: выражением `"low" if effort == "medium" else "medium"`, и оба экземпляра
#: делали одно и то же: с нижней ступени поднимали обдумывание **обратно
#: вверх** — а попадали в них ровно тогда, когда предыдущая попытка уже
#: задохнулась в размышлении. Пока обе стартовали с `None`, это читалось как
...
def at_the_bottom(effort: str | None) -> bool:
    ...
    Повторить неизменившийся запрос — это купить тот же отказ второй раз: те же
    токены, то же усилие, тот же результат, за те же деньги и то же ожидание.
    """
    return effort == EFFORT_LADDER[-1]


def turn_down(effort: str | None) -> str:
    """Следующая ступень вниз. С последней — она же."""
    if effort not in EFFORT_LADDER:
        return EFFORT_LADDER[0]
    return EFFORT_LADDER[min(EFFORT_LADDER.index(effort) + 1, len(EFFORT_LADDER) - 1)]
...
class AnthropicProvider:
    ...
    def __init__(self, api_key: str | None = None) -> None:
        ...
        #
        # **Числа таймаутов названы все четыре, и `None` среди них нет.**
        #
        # Здесь стояло `httpx.Timeout(None, connect=5.0)` с доводом «читать
        # ответ мы обязаны сколько угодно, длинная глава наблюдалась три
        # минуты». Довод не про то: `read` у httpx — это ожидание **одного
        # чтения из сокета**, а не всего ответа, и трёхминутная глава его не
        # трогает, потому что байты идут потоком. Зато `None` означает, что
        # собеседник, доведший рукопожатие TLS и замолчавший, держит задачу
        # вечно; соединение не возвращается в пул из сорока, а `pool=None`
        # заставляет сорок первую генерацию ждать его тоже вечно. Один
        # молчаливый узел на той стороне — и воркер деградирует навсегда,
        # причём тихо: `timeout` у gunicorn такую задачу не снимет, uvicorn
        # стучит в его пульс независимо от того, идёт ли работа.
        #
        # Свой клиент нужен ради пула: `AsyncAnthropic` без `http_client`
        # собирает пять keep-alive соединений, а воркер ведёт больше генераций
        # сразу (пул базы 20, поток глав и чат идут параллельно), и шестая
        # начинала бы с рукопожатия — два круга по сети перед первым токеном,
        # который человек ждёт на экране. Но задав свой клиент, мы отменяем и
        # умолчания SDK (600/600/600), поэтому их приходится назвать заново.
        self._client = AsyncAnthropic(
            api_key=key,
            http_client=httpx.AsyncClient(
                timeout=httpx.Timeout(600.0, connect=5.0, pool=30.0),
                limits=httpx.Limits(
