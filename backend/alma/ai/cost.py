...
MEASURED_OVER_PROJECTED: dict[str, float] = {
    ...
    "opening": 2.05,
    "chat_turn": 2.40,
    # Перевод — единственная статья без замера: заведена 28.08.2026 вместе с
    # `ai/translator.py`, живых переводов ещё ноль. 1.5 — запас, а не измерение:
    # у перевода нет цикла попыток (одна, редко две) и форма ответа задана
    # исходником, то есть оба источника, из которых прочие статьи набрали свои
    # ×1.8–2.4, здесь почти отсутствуют. Пересчитать по первой сотне строк:
    #   select count(*), avg(cost_cents) from reading
    #     where model like 'claude-haiku%';            -- переводы глав
    "translation": 1.5,
}
