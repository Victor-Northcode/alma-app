...
def build_prompt(
    ...
    *,
    ...
    lines = [
        ...
        # Два правила в одной строке, и строка нарочно короткая: промпт
        # бесплатной главы стоит впритык к потолку $0.05, и первая, более
        # многословная редакция дважды роняла его в BudgetExceeded. «Как
        # написано» — из «0°94′» на живом экране: орб 0.94° пересчитан в
        # минуты, которых не бывает больше 59; «never invent» — из «накала
        # страстей N» там же.
        "NUMBERS: copy figures exactly as the data writes them; never "
        "invent or convert numbers.",
        ...
        *(
            (
                ...
                if reader_gender in ("female", "male")
                    ...
                    "«от тебя требуется»). Прошедшее время о читателе запрещено "
                    # «что собиралась сделать», «уже однажды решила» — живой
                    # текст дня 29.08.2026: род просочился в придаточных без
                    # «ты», куда узкий список валидатора не смотрит.
                    "целиком, даже без «ты» («что собиралась сделать», «уже "
                    "однажды решила» — то же нарушение): переписывай в настоящее. "
                    "Латиницей в тексте может быть только "
