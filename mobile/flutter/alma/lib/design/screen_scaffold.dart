...
class ScreenScaffold extends StatelessWidget {
  ...
  /// Отступ списка сверху. Назван, потому что экраны, считающие свою высоту
  /// сами, обязаны считать её теми же числами, что и каркас.
  static const listTop = 12.0;

  /// Воздух под последней строкой, сверх высоты бара вкладок.
  ///
  /// **Было 26, и на двух экранах это стоило неподвижности.** Числа этого
  /// каркаса складываются с числами страницы, и страница, спроектированная «в
  /// край», от лишнего воздуха переваливает за кромку: «Сегодня» без подписки
  /// уезжал на 33 точки, колода «Моих систем» — на 7. Владелец увидел это как
  /// дёрганье: «экран today и my systems дёргается, хотя там некуда листать».
  /// Экран, который **влезает**, не имеет права ездить вовсе, и лишний воздух
  /// под ним — не украшение, а причина, по которой он ездит.
  ///
  /// Шестнадцать — это `AlmaMetrics.gap`: последняя строка по-прежнему стоит
  /// на ясном расстоянии от растворяющейся кромки бара, а не под ней. Полная
  /// высота бара вместе с домашним индикатором зарезервирована отдельно и
  /// осталась нетронутой — ничего не обрезается.
  static const bottomAir = AlmaMetrics.gap;
  ...
  Widget build(BuildContext context) {
    ...
    Widget list = ListView(
      ...
      //
      // **`AlwaysScrollable` не пружинит — пружинит родитель.** Здесь родитель
      // `Clamping`, и список, которому некуда ехать, не едет ни на точку: жест
      // принимается только затем, чтобы его увидел `RefreshIndicator`. Когда
      // владелец нашёл дёрганье на «Сегодня» и «Моих системах», причиной была
      // не физика, а высота: обе страницы переваливали за кромку на десятки
      // точек и ездили по-настоящему. Чинится это высотой, а не физикой, —
      // иначе обновление жестом, которое на «Сегодня» стоит намеренно, ушло бы
      // вместе с дёрганьем.
      ...
      padding: EdgeInsets.only(
        ...
        //
        // Бар рисует себя как `tabBarHeight` плюс домашний индикатор
        // (`tab_bar.dart`), и содержимое уходит под него: он растворяется
        // кверху, а не стоит панелью. Значит списку нужно ровно это плюс воздух.
        //
        // **Отступ считал бар дважды.** Раньше здесь стояло
        // `tabBarHeight + MediaQuery.paddingOf(context).bottom`, и это выглядело
        // как «высота бара плюс домашний индикатор» — но оболочка объявлена
        // `Scaffold(extendBody: true, bottomNavigationBar: …)`, а такой Scaffold
        // **уже** кладёт в `MediaQuery` тела полную высоту бара вместе с
        // индикатором. Измерено на устройстве: `ambient=86`, `window=34`. То
        // есть 52 прибавлялись к 86, и под последней строкой оставалось 164
        // точки пустой ночи вместо 86 — почти вдвое больше нужного.
        //
        // Считаем от окна, а не от окружения: окно не зависит от того, что с
        // отступом сделали Scaffold, SafeArea или вложенный маршрут по дороге.
