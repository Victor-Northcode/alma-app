...
/// потоками.
///
/// **Почему `Int64`, а не обычный `int`.** На телефоне `int` в Dart — это те же
/// 64 бита с обёрткой, что и в Swift, и код работал. В браузере `int` — это
/// число с плавающей точкой: константа `0x9E3779B97F4A7C15` туда просто не
/// помещается, и сборка под веб падает на этой строке. Веб поймал разницу
/// платформ, которую телефон бы скрыл.
///
/// `Int64` даёт точные 64 бита везде — на телефоне, в браузере и в тесте. Это
/// стоит нескольких наносекунд на звезду, а звёзд 160 и генерируются они один
/// раз на экран. Взамен последовательность выходит **побитово та же, что на
/// iOS**, и небо совпадает звезда в звезду на любой платформе.
class SkyRandom {
  Int64 _state;
  ...
  SkyRandom({int seed = 0x414C4D41})
      ...
      : _state = Int64(seed) + _golden;

  static final Int64 _golden = Int64.parseHex('9E3779B97F4A7C15');
  static final Int64 _mixA = Int64.parseHex('BF58476D1CE4E5B9');
  static final Int64 _mixB = Int64.parseHex('94D049BB133111EB');

  Int64 next() {
    _state = _state + _golden;
    var z = _state;
    z = (z ^ z.shiftRightUnsigned(30)) * _mixA;
    z = (z ^ z.shiftRightUnsigned(27)) * _mixB;
    return z ^ z.shiftRightUnsigned(31);
  }

  double unit() =>
      next().shiftRightUnsigned(11).toDouble() * (1.0 / 9007199254740992.0);
