...
/// Ответ на запрос главы — **один и тот же для открытой и закрытой**.
///
...
/// (платить сильной моделью за целую главу до покупки), и какое-то время
/// закрытая глава не спрашивала сервер вовсе: право читалось из оглавления, а
/// экран рисовал стену.
///
/// **Теперь она спрашивает снова — но не главу, а её первый абзац.** Сервер
/// отвечает 200 с `locked: true`, пустым [reading] и [opening]: ≈40 слов,
/// написанных средней моделью из настоящих позиций, единственные токены до
/// покупки (`readings.py`, `_locked_chapter`). Из них закрытая глава и делает
/// то, чего не делала стена: доказывает, что текст про этого человека.
///
/// `reading` присутствует и равен `null` намеренно — так его шлёт сервер.
/// Форма ответа одна на оба случая, и «главы здесь нет» сказано полем, а не его
...
class ReadingResponse {
  ...
  /// Написанная глава целиком. `null` — её здесь нет, см. [locked].
  final Reading? reading;

  /// Открывающий абзац закрытой главы, той же формы, что и сама глава: у него
  /// есть заголовок, тело и — главное — `citedFactors`, то есть позиции, из
  /// которых он прочитан. Мета-строка «read from» на закрытой главе берётся
  /// отсюда.
  ///
  /// `null` — законное состояние, а не сбой: модель могла промолчать, потолок
  /// месяца мог быть выбран, анкеты могло не быть. Экран рисует паттерн без
  /// абзаца, а не падает и не подставляет вместо него филлер.
  final Reading? opening;

  /// Права на главу нет. Слово сервера, а не догадка клиента.
  final bool locked;

  /// Закрыто тем, что пару нечем назвать, — единственное «закрыто», которое не
  /// чинится деньгами. Экран просит человека, а не цену.
  final bool needsPartner;

  /// Ключ полки, которым эта система открывается (`door.natal`, `sub.monthly`,
  /// `pair.check`). Слово каталога сервера — им и надо торговать, а не тем, что
  /// клиент вывел из слага системы.
  ...
  static Reading? _maybe(Object? raw) => raw is Map
      ? Reading.fromJson(raw.cast<String, dynamic>())
      : null;

  factory ReadingResponse.fromJson(Map<String, dynamic> json) => ReadingResponse(
        reading: _maybe(json['reading']),
        opening: _maybe(json['opening']),
        locked: json['locked'] as bool? ?? false,
        needsPartner: json['needs_partner'] as bool? ?? false,
...
enum FunnelStage {
  ...
  /// **Веб-ступень. Телефон её не шлёт — и не должен.**
  ///
  /// Живёт здесь потому, что это перечисление — зеркало словаря сервера
  /// (`alma/funnel.py::STAGES`), и тест сверяет их целиком; выкинуть значение
  /// значило бы уронить сверку, а не убрать ступень.
  ///
  /// Пишет её сервер, открывая сессию Paddle для браузера. Телефонная половина
  /// — [checkoutStarted] ниже. Приложение слало её из `AlmaStore.buy` до
  /// 17 августа 2026, и вред был не в лишней строке: родителя `offer_view`
  /// телефон не шлёт вовсе, так что каждая покупка надувала числитель
  /// веб-этапа при неизменном знаменателе, а от той же ступени считается и
