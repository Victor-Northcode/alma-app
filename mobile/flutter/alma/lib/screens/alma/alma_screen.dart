...
class _Turn {
  ...
  ///
  /// Есть и у живого ответа, и у поднятой с сервера беседы: сервер хранит
  /// тройку в самом сообщении и отдаёт её в архиве тем же полем. Раньше её не
  /// было только у второго, и дверь в главу пропадала при перечитывании треда —
  /// при том что ответ на эту главу ссылался словами.
...
class _AlmaScreenState extends State<AlmaScreen>
  ...
  Future<void> _send(String text) async {
    ...
    // Упёрлись ли в квоту — признак, а не сразу вызов: экран квоты открывается
    // **после** `finally`, и почему именно так, сказано там же, у вызова.
    var quotaHit = false;
    setState(() {
      _turns.add(pending);
      _refusal = null;
    ...
    // **Экран квоты приходит сразу, а не по нажатию на стену.** ТЗ P6: триггер
    // — отправленный четвёртый вопрос, а не интерес к цене. Мягкий экран
    // отвечает на действие, которое человек только что совершил, и держит в
    // руках то, ради чего он его совершил.
    //
    // **Но открывается он за `finally`, а не в `catch`.** Стоял он там — то
    // есть внутри отправки, которая ещё не закрыта, — и это стоило человеку
    // ровно того, за что он заплатил. Купивший возвращается сюда, `_openQuota`
    // отправляет удержанный вопрос тем же [_send], а тот первой же строкой
    // видит `_sending == true` (снимается оно ниже по стеку, в `finally`) и
    // молча выходит. Вопрос при этом уже стёрт — и из удержания, и из поля, —
    // так что человек, только что оплативший конкретный ответ, оставался с
    // пустым экраном и без своих слов. Худший момент продукта для потери.
    if (quotaHit && mounted) await _openQuota(message);
  }
