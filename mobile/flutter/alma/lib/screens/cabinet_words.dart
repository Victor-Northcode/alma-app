...
extension CabinetWordsMore on CabinetWords {
  ...
  /// Попадание транзита, названное по-человечески.
  ///
  /// **Зачем отдельно от [factor].** Тот писался под натальную позицию —
  /// «venus 28°40′ ♓ · 7th house», — и переводит тела, знаки и дома. У транзита
  /// форма другая: сервер строит её в `transits.py::describe()` как
  /// `transiting {тело}[ retrograde] {глиф} natal {тело} · orb {N}°`. Через
  /// [factor] она проходила наполовину: тела становились русскими, а служебные
  /// «transiting» и «natal» оставались английскими, и в читалке гороскопа
  /// стояло «transiting Уран □ natal Меркурий · orb 0.85°». Найдено глазами.
  ///
  /// **Служебные слова выброшены, а не переведены.** «Транзитный Уран к
  /// натальному Меркурию» — это язык таблицы, а не разговора; на экране, который
  /// называется «твоё небо», обе роли и так известны, и порядок их держит:
  /// первым идёт то, что движется, вторым — то, что стоит в карте. Орбис в
  /// сотых долях градуса — довод внутренней кухни, ему на карточке места нет.
  ///
  /// Ретроградность остаётся: она меняет смысл, и слово для неё в каталоге уже
  /// есть. Глиф аспекта остаётся глифом по той же причине, что и в [factor].
  ///
  /// Строку, не похожую на транзит, отдаём [factor] — он разберётся или вернёт
  /// как есть.
  static final _hit = RegExp(
      r'^transiting\s+([a-z_]+)(\s+retrograde)?\s+(\S+)\s+natal\s+([a-z_]+)');

  static String hit(L l, String raw) {
    final m = _hit.firstMatch(raw.trim());
    if (m == null) return factor(l, raw);
    final moving = CabinetWords.body(l, m.group(1)!);
    final still = CabinetWords.body(l, m.group(4)!);
    final back = m.group(2) == null ? '' : ' ${l.dailyRetrograde}';
    return '$moving$back ${m.group(3)} $still';
  }
