            ...
            child: Column(
              ...
              children: [
                ...
                if (widget.opening case final opening?)
                  ...
                  // настоящее.
                  //
                  // **Кроме пары.** У совместимости открывающего абзаца нет по
                  // замыслу — сервер отвечает ей стеной без текста первой же
                  // веткой («у пары бесплатного текста нет вообще»,
                  // `readings._locked_chapter`), — и «что-то пошло не так»
                  // здесь было бы ложным признанием поломки на каждой паре у
                  // каждого человека. Её кадр C6 говорит медальонами и
                  // вопросом главы. Найдено при съёмке кадров витрин
                  // (27.08.2026): golden показал ветку отказа там, где отказа
                  // не было.
                  if (widget.system != SystemSlug.compatibility)
                  Padding(
