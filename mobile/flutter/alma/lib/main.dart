...
class _CabinetShellState extends State<CabinetShell> {
  ...
  Future<void> _goTo(CabinetTab tab) async {
    if (tab == _tab) {
      // Тап по вкладке, на которой уже стоишь. На «Моих системах» это выход
      // к корню стека: человек, стоящий внутри натальной карты, жмёт «Мои
      // системы» и ждёт список систем, а не тишину (владелец, 25.08.2026).
      // На Alma это единственный способ сказать бару «спасибо, показал».
      if (tab == CabinetTab.systems) {
        _systemsNav.currentState?.popUntil((route) => route.isFirst);
      }
      _peek.hide();
      return;
    }
