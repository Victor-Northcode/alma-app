...
/// Зачем открыли витрину: за одной дверью или за всем сразу.
///
/// Порт `PaywallIntent`. Разница не косметическая: она решает, что стоит первой
/// ступенью и что выбрано заранее.
class PaywallIntent {
  const PaywallIntent.door(SystemSlug this.system);
  const PaywallIntent.everything() : system = null;
...
/// * без двери первым идёт год, под ним неделя и месяц: на годовой приходится
///   60–70% продаж плана, когда он ведёт;
...
List<LadderKey> ladderFor({
  ...
  if (!held.hasPlan && !intent.isDoor) {
    keys.addAll(const [LadderKey.annual, LadderKey.weekly, LadderKey.monthly]);
  }
  if (!held.ownsArchive) {
