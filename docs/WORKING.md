...
**Валюта цены — это страна Apple Account, а не место и не регион телефона.**
Владелец дважды приносил «цена в рублях» (26.08 из евро-региона, 27.08 из
Испании). Это не баг: приложение печатает цену ровно так, как её отдаёт
StoreKit, а StoreKit берёт витрину из страны вошедшего Apple Account —
российский аккаунт видит рубли в любой точке мира, и списание тоже будет в
рублях. Испанец с испанским аккаунтом видит евро сам. Рисовать «местную»
валюту по геолокации нельзя: число на кнопке обязано совпасть со списанным.
Посмотреть чужую витрину — sandbox-тестер с нужной страной (App Store
Connect → Users and Access → Sandbox) на отладочной сборке; ревьюеры Apple
видят витрину USA. Отдельный случай: сборка, запущенная из Xcode кнопкой
Run, говорит с локальным тест-магазином `ios/Alma.storekit` — там валюта
идёт от **региона устройства**, и это тоже не то, что увидит покупатель.
...
**Правка строк клиента ломает тесты сервера.** `backend/tests/test_soft_hyphens.py`
читает `mobile/flutter/alma/lib/l10n/app_*.arb` напрямую и требует мягкого
переноса внутри длинных слов — «Geburtshoroskop», «астрокартография»: без него
слово выпирает за карточку на узком телефоне, и ловится это не глазами, а
здесь. Такие же тесты есть на французские пробелы и на совпадение словарей.

Поэтому после **любой** правки `.arb` гонять обе стороны: `flutter analyze` и
`flutter test` — и `backend/.venv/bin/python -m pytest tests -q`. Я один раз
запушил строки, проверив только порт, и увёл в main два красных теста; нашёл их
соседний агент, а не я.
