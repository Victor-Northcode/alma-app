...
**Уточнения по факту реализации (Ф0.3/Ф0.4):**

* `POST /billing/iap/verify` отвечает **202** и `status: "unbound"`, когда покупка пары приехала без токена в подписанном пейлоаде. Тот же ответ приходит и на повтор такой транзакции — пока грант не выписан, состояние не меняется;
* `GET /billing/credits` → `{"pair": {granted, used, remaining, period_end}}`. `remaining` считает сервер: клиенту нечем — он не знает границ расчётного периода и не видит трат с других устройств;
* **отдельного эндпоинта «потратить кредит» нет и не нужно.** Кредит тратится там, где возникает расход, — на первой платной главе про нового человека в `POST /readings`; попытка сверх включённой отвечает 402 `beyond_monthly_pair`. Отдельная ручка означала бы, что «включённая проверка» списывается по слову клиента, а не по факту генерации;
* тизер сверх капа отвечает 402 `teaser_cap` из того же `POST /readings`, до генерации.
...
|---|---|---|---|
| Тизер «Притяжение» | mid | ~2 абзаца, ~3¢ | внутри `free_month_budget`; кап **1 новый человек/мес** на free-тир |
...
**Кап — один человек в месяц** (решение владельца от 17.08.2026; холст с «Three free previews this month» устарел). Значение живёт в `Setting["pair.teaser_cap"] = {"per_month": N}` и меняется без релиза; дефолт — `credits.TEASER_CAP_DEFAULT`. Считаются **новые люди**, а не запросы: перечитывание уже написанного тизера бесплатно всегда и в счёт не идёт. Освобождены владелец оплаченного отчёта про этого человека и подписчик.
